...
public class Principall extends javax.swing.JFrame {
    ...
    public void LoadTableFunc(){
        DefaultTableModel modelo = new DefaultTableModel(new Object[]{"Codigo","Nome","Cadastro"},0);
            
        for(int i=0;i<ListaFunc.size();i++){
            Object linha[] = new Object[]{ListaFunc.get(i).getCodigo(),
    ...
    private void initComponents() {
        ...
        jPanel4.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(0, 0, 0)), "Usuario"));
    ...
    private void btn_dep_salvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_dep_salvarActionPerformed
        ...
        if(modoDep.equals("Novo")){  
            Cadastro D = new Cadastro(cod, c_dep_nome.getText());
            ListaDep.add(D);
        }else if(modoDep.equals("Editar")){
            int index = tbl_dep_dpts.getSelectedRow();
            ListaDep.get(index).setId(cod);
    ...
    private void tbl_dep_dptsMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbl_dep_dptsMouseClicked
        ...
        if(index>=0 && index<ListaDep.size()){
            Cadastro D = ListaDep.get(index);
            c_dep_codigo.setText(String.valueOf(D.getId()));
    ...
    private void btn_func_salvarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_func_salvarActionPerformed
        ...
        if(index==0){
            JOptionPane.showMessageDialog(this,"Você deve fazer o Cadastro");
        }else{
            Usuario F = new Usuario();
            F.setCodigo(Integer.parseInt(c_func_mat.getText()));
